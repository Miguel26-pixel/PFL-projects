...
game_over([]/Turn/_,Turn) :- !.
game_over([Row | T]/Turn/_Players,Winner) :-    game_over_row(Row,Res),
                                                (Res =:= 0 -> 
                                                    Winner is 0 
                                                ; 
                                                    game_over(T/Turn/_Players,Winner)
                                                ).


valid_moves(Board/Turn/Players,ValidGameState) :-   valid_moves(Board,Board,0,Turn,[],Moves),
                                                    ValidGameState = Moves/Turn/Players.
...
get_valid_move(Moves/_Turn/_Players,Row/Col) :- get_move(R,C), 
                                                check_move(Moves,R,C,Valid),
                                                ( Valid =:= 0 -> 
                                                    write('ERROR: Invalid Move'), nl,
                                                    get_valid_move(Moves/_Turn/_Players,Row/Col)
                                                ; 
                                                    Row is R,
                                                    Col is C
                                                ).
